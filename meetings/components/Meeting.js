class Meeting extends React.Component {
    ...
    loadMeeting() {
        $.get(`/api/getMeetingNew?meetingID=${this.props.id}`, this.handleMeeting)
    }
    ...
    handleMeeting(data) {
        ...
        if (data.code === 200) {
            ...
            this.setState({
                loaded: {points: true},
                points: [{id: '0', name: 'Failed To Load Meeting', author: 'Reload the page to try again.'}]
            })
    ...
    handleAddPoint = (data) => {
        ...
        if (parseInt(data.meetingID) === parseInt(id)) {
            ...
            let newState = [{name: data.name, author: data.author, id: data.id}, ...prevState];
    ...
    loadPointDetails(id) {
        ...
        $.get(`/api/getPointNew?pointID=${id}`, this.handleGetPoint);
    ...
    handleGetPoint(data) {
        data = JSON.parse(data);
        if (data.code === 200) {
            this.setState({point: data.response, loaded: {...this.state.loaded, point: true}});
        }
    }

    toggleCreateNewPoint() {
        this.setState({open: {createNewPoint: !this.state.open.createNewPoint}});
    }

    handleChangePTitle(d) {
        this.setState({new: {...this.state.new, point: {...this.state.new.point, title: d.target.value}}});
    }

    handleChangePDescription(d) {
        this.setState({new: {...this.state.new, point: {...this.state.new.point, description: d.target.value}}});
    }

    handleChangeCContent(d) {
        this.setState({new: {...this.state.new, comment: {content: d.target.value}}});
    }

    handleFormSubmit(e) {
        e.preventDefault();
        if (this.state.new.point.description !== '' && this.state.new.point.title !== '') {
            $.post('/api/addPointNew', {
    ...
    handleAddedPoint() {
        this.setState({new: {...this.state.new, point: {title: '', description: ''}}});
    }
    ...
    render() {
        return (
            <div>
                <button onClick={this.toggleCreateNewPoint}
                        className={"newPointBtn " + (this.state.open.createNewPoint ? 'open' : '')}>
                    +
                </button>
                <div className={"grid new meeting " + (this.state.open.createNewPoint ? 'open' : '')}>
                    <div className="grid__col grid__col--2-of-6">
                        <div className="gridLeftPadding">
                            ...
                            <div className="selectionPanel meetingPanelHeight">
                                {(this.state.loaded.points) ? (this.state.points.map((point) => (
                                    <li key={point.id} className="selectionTab points"
                                        onClick={() => this.loadPointDetails(point.id)}>
                                        <h2 dangerouslySetInnerHTML={{__html: point.name}}/>
                                        <p>{point.author}</p>
                                        <span className="delPoint" title="Double Click To Delete."
                                              onDoubleClick={() => this.deletePoint(point.id)}>Delete Point</span>
                                    </li>
                                ))) : (
                            ...
                            <div className="infoPanel">
                                {this.state.loaded.point ? (
                                    <div className="pointControl" style={{left: '66.15%'}}>
                                        <h1 dangerouslySetInnerHTML={{__html: this.state.point.name}}/>
                                        <small>{this.state.point.author}</small>
                                        <p className="description" dangerouslySetInnerHTML={{__html: this.state.point.description}}/>
                                        <div className="comments">
                                            {this.state.point.comments.length ? (this.state.point.comments.map((comment) => (
                                        ...
                                        <form onSubmit={this.handleNewCommentSubmit}>
                                            <div className="field addComment">
                                                <div className="fieldTitle">Your Comment <button
                                                    className="postComment">Post</button></div>
                                                <input type="text" className="fieldInput"
                                                       value={this.state.new.comment.content}
                                                       placeholder="Your Comment" onChange={this.handleChangeCContent}/>
                                            </div>
                                        </form>
                                    </div>
                                ) : (
                ...
                <div className={"drawer " + (this.state.open.createNewPoint ? 'open' : '')}>
                    <h1>New Point</h1>
                    <form onSubmit={this.handleFormSubmit}>
                        <div className="field">
                            <p className="fieldTitle">Point Title</p>
                            <input className="fieldInput" value={this.state.new.point.title} type="text"
                                   placeholder="Point Title" onChange={this.handleChangePTitle}/>
                        </div>
                        <div className="field">
                            <p className="fieldTitle">Point Description</p>
                            <textarea className="fieldTextarea pdesc" placeholder="Point Description"
                                      onChange={this.handleChangePDescription}
                                      value={this.state.new.point.description}/>
                        </div>
                        <button type="submit" className="createPointBtn">Create</button>
                    </form>
                </div>
            </div>
        )
